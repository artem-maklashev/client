...
const DelaysMonthChartBoard: React.FC<DelaysChartBoardProps> = ({ delays }) => {
    ...
    useEffect(() => {
        if (data) {
            ...
            data.sort((a, b) => new Date(a.delayDate).getTime() - new Date(b.delayDate).getTime()).forEach((item) => {
                const dateStr = new Date(item.delayDate).toISOString().split('T')[0].substring(0, 7);
                const existingEntry = groupedData.find((entry) => entry.date.substring(0, 7) === dateStr);
    ...
    const CustomLegend: React.FC<CustomLegendProps> = (props) => {
        ...
        return (
            <div style={{ fontSize: '12px', textAlign: 'center', background: 'rgba(0, 0, 0, 0)'}}>
                {payload.map((entry) => {
    ...
    const CustomTooltip: React.FC<TooltipProps<ValueType, NameType>> = ({ active, payload = [], label }) => {
        ...
        return (
            <div style={{ fontSize: '14px', textAlign: 'center', background: 'rgba(250, 248, 248, 0.8)', fontFamily: 'sans-serif', borderRadius: 8 }}>
    ...
    const handleClick = (chartData: CombinedData | undefined) => {
        if (chartData) {
            ...
            const filteredDelays = data.filter((delay) => new Date(delay.delayDate).toISOString().split('T')[0].substring(0, 7) === date)
...
export default DelaysMonthChartBoard;
